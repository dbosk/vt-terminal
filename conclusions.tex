...
as the GUI}).
Every aspect has an item in the pre/post-test of \cref{app:quiz} (the
first has an open twin as well, since its closed form is asked only at
the end), which is what will turn \enquote{candidate} into
...
\begin{table}
  \begin{sidecaption}{The candidate critical aspects, what each is read
    ...
    \begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.36\linewidth}l>{\raggedright\arraybackslash}p{0.34\linewidth}@{}}
      ...
      The pipe is not the file & literature, course & sending output into a pipe; a file on the pipe \\
      ...
      A shell runs somewhere & course & which computer answers \\
