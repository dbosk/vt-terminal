...
The first kind is closed: essentially one graded item per candidate
critical aspect (\cref{tab:aspects}),
...
Four ungraded items ask the students for their own account of a shared
stimulus---what the terminal is and what happens when a command is
entered; how they would get a file from one folder to another; what goes
on in a pipeline; what has happened after logging in to a server and how
results get back to the laptop.
...
whose prompts are the four open stems followed up in conversation, with
